...
if ($response['status']) {
$response['message'] = sprintf('با استفاده از پی اچ پی v%s', $response['version']);
} else {
$response['message'] = sprintf('در حال حاضر از PHP نسخه v%s استفاده می‌شود. لطفاً از PHP نسخه ۷ یا بالاتر استفاده کنید.', $response['version']);
}
break;
case 'php-extensions':
...
if ($response['status']) {
$response['message'] = sprintf('حداکثر زمان اجرا %s', ini_get('max_execution_time').' ثانیه');
} else {
$response['message'] = sprintf('لطفا حداکثر زمان اجرای خود را افزایش دهید.' );
$response['description'] = '</span>این مشکل را می‌توان به سادگی با <p><a href="https://www.simplified.guide/php/increase-max-execution-time" target="_blank"> افزایش حداکثر زمان اجرای</a> و رساندن آن به ۶۰ یا بیشتر و راه‌اندازی مجدد سرور پس از اعمال این تغییر، رفرش کردن مرورگر و دوباره امتحان کردن، حل کرد.</p>';
}
break;
...
if ($response['status']) {
$response['message'] = sprintf('مجوز خواندن/نوشتن برای فایل ‎.env‎ فعال شد.');
} else {
$response['message'] = sprintf('لطفا مجوز خواندن/نوشتن را برای فایل <b>.env</b> پروژه خود فعال کنید.');
$response['description'] = '</span> مشکل را می‌توان به سادگی با فعال کردن <a href="https://www.uvdesk.com/en/blog/open-source-helpdesk-installation-on-ubuntu-uvdesk/" target="_blank"><p> فعال کردن مجوز خواندن/نوشتن فایل <b>.env</b></a>، رفرش کردن مرورگر و دوباره امتحان کردن، حل کرد.</p>';
}
break;
...
'description' => '</span> <br><p> این مشکل را می‌توان به سادگی با فعال کردن مجوزهای خواندن/نوشتن برای فایل‌های خود در پوشه config/packages پروژه <a href="https://www.uvdesk.com/en/blog/open-source-helpdesk-installation-on-ubuntu-uvdesk/" target="_blank"> حل کرد.</a></p>',
];
break;
case 'redis-status':
...
'status' => false,
'message' => "افزونه Redis روی سرور شما نصب شده است، جزئیات را دنبال کنید",
'description' =>'<span>لطفاً <a href="https://github.com/uvdesk/community-skeleton/issues/364#issuecomment-780486976" target="_blank">میزبان Redis</a> مشخص شده در فایل <span style="font-weight:600;">setup.php</span> را بررسی کنید. اگر میزبان سرور Redis شما متفاوت است، می‌توانید آن را با میزبان Redis خود به‌روزرسانی کنید یا مراحل موجود در <a href="https://github.com/uvdesk/community-skeleton/issues/364#issuecomment-780486976" target="_blank">URL</a> را دنبال کنید. پس از ایجاد تغییرات، صفحه را مجدداً بارگذاری کنید و دوباره امتحان کنید.</span>'
]);
}
break;
default:
...
throw new \Exception('پایگاه داده وجود ندارد.');
}
// Create database
