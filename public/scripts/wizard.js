...
errorFlag = true;
this.$el.find('.form-content input[name="memberUrlPrefix"]').after("<span class='wizard-form-notice'>فقط حروف و اعداد مجاز هستند</span>")
}
if (! prefixTestRegex.test(customerPrefix)) {
errorFlag = true;
this.$el.find('.form-content input[name="customerUrlPrefix"]').after("<span class='wizard-form-notice'>فقط حروف و اعداد مجاز هستند</span>")
}
}
if (false == errorFlag) {
this.wizard.enableNextStep();
...
errorFlag = true;
this.$el.find('input[name="name"]').parent().append("<span class='wizard-form-notice'>نام نامعتبر</span>")
}
}
if (
! errorFlag 
...
errorFlag = true;
this.$el.find('input[name="email"]').parent().append("<span class='wizard-form-notice'>ایمیل نامعتبر</span>")
}
}
if (
...
errorFlag = true;
this.$el.find('input[name="password"]').parent().append("<span class='wizard-form-notice'>رمز عبور باید حداقل شامل ۸ کاراکتر، حداقل دو حرف (به حروف کوچک و بزرگ حساس نیست)، یک عدد و یک کاراکتر ویژه (فاصله مجاز نیست) باشد.</span>")
}
if (
...
errorFlag = true;
this.$el.find('input[name="confirm_password"]').parent().append("<span class='wizard-form-notice'>رمز عبور مطابقت ندارد.</span>")
}
if (
! errorFlag 
&& (user.name == null
...
this.view.$el.find('.form-content input[name="createDatabase"]').parents('.form-content').append("<span id='wizard-error-id' class='wizard-form-notice'>جزئیات نادرست است! اتصال برقرار نشد.</span>");
wizard.disableNextStep();
}
}.bind(this)).fail(function(response) {
wizard.disableNextStep();
...
status: false,
message: ERRORS.hasOwnProperty('error' + jqXHR.status) ? ERRORS['error' + jqXHR.status].title : 'خطای غیرمنتظره‌ای در فرآیند تأیید نسخه PHP رخ داد.',
...
status: false,
message: ERRORS.hasOwnProperty('error' + jqXHR.status) ? ERRORS['error' + jqXHR.status].title : 'خطای غیرمنتظره‌ای در فرآیند ارزیابی افزونه PHP رخ داد',
description: ERRORS.hasOwnProperty('error' + jqXHR.status) ? ERRORS['error' + jqXHR.status].description : 'جزئیات موجود نیست',
});
}).always(() => {
...
status: false,
message: ERRORS.hasOwnProperty('error' + jqXHR.status) ? ERRORS['error' + jqXHR.status].title : 'خطای غیرمنتظره‌ای در طول فرآیند تأیید حداکثر زمان اجرا رخ داد',
description: ERRORS.hasOwnProperty('error' + jqXHR.status) ? ERRORS['error' + jqXHR.status].description : 'حداکثر زمان اجرا</span><p>برای حل این مشکل می‌توانید این لینک وبلاگ را مطالعه کنید:<a href="https: //www.simplified.guide/php/increase-max-execution-time" target="_blank">نحوه حل مشکل افزونه mailparse در PHP</a></p>',
});
}).always(() => {
...
var currentExtensionTextStatus = "<span class='extension_name'>" + currentExtensionName + "</span> افزونه در حال حاضر فعال است.";
} else {
...
extension_info.find('.extension-criteria-label').html("شما الزامات " + activeExtensionCount + " از " + extensionCount + " افزونه‌های PHP را برآورده می‌کنید.");
} else {
...
var currentConfigFileTextStatus = "<span class='configfiles_name'>" + currentConfigFileName + ".yaml </span> مجوز خواندن/نوشتن فایل فعال است.";
} else {
var currentconfigfileIconStatus = this.wizard_icons_notice_template();
if (currentConfigFileName == 'uvdesk') {
var currentConfigFileTextStatus = "<span class='configfiles_name'> " + currentConfigFileName + ".yaml مجوز خواندن/نوشتن فایل غیرفعال است </span>";
...
configFile_info.find('.permissionConfigfiles-criteria-label').html("شما با مجوز " + activeconfigfileCount + " از مجوزهای فایل‌های پیکربندی " + configfileCount + " مواجه می‌شوید.");
} else {
